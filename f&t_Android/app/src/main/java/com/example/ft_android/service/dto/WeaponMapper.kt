...
class WeaponMapper {
    ...
    /*fun map(dto: List<WeaponDTO>?): List<Weapon>? {
        val weaponList: MutableList<Weapon> = mutableListOf()

        dto?.let { weaponDTOList ->
            for (weaponDTO in weaponDTOList) {
                weaponList.add(mapWeapon(weaponDTO))
            }
        }
